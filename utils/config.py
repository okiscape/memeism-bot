...
if _truthy(env.get("PROXY_DISABLED")) or _truthy(env.get("NO_PROXY")):
    ...
    proxy_url = (
        ...
        and str(_raw_proxy).strip().lower()
        not in ("none", "null", "off", "false", "0", "-")
        else None
    )

postgres_dsn = (
    f"postgresql://{env.get('POSTGRES_USER', 'memeism')}"
    f":{env.get('POSTGRES_PASSWORD', '')}"
    f"@{env.get('POSTGRES_HOST', 'localhost')}"
    f":{env.get('POSTGRES_PORT', '5432')}"
    f"/{env.get('POSTGRES_DB', 'memeism')}"
)
...
version_name = env.get("BOT_VERSION_NAME") or "kisa"
