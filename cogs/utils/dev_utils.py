...
class DevUtilsCog(cog.Cog):
  ...
  async def on_interaction(self, inter: cog.Interaction):
    ...
    if inter.type == cog.InteractionType.application_command:
      ...
      if inter.guild:
        guinfo = f"Guild name: '{inter.guild.name}' | Guild id: {inter.guild.id}"
      else:
        guinfo = "DM"
      ...
      self.bot.logging.info(f"{guinfo} | Author: {inter.author.global_name or inter.author.name} | {inter.author.id} | Args: {inter.data['name']}{sub}{arguments}", type="execute command")
    ...
    if inter.type == cog.InteractionType.component:
      ...
      try:
        ...
        self.bot.logging.info(f"{guinfo} | Author: {inter.author.global_name or inter.author.name} | {inter.author.id} | Cus-id: {inter.data['custom_id']} | Values: {inter.data['values']}", type="execute component")
      except:
        ...
        self.bot.logging.info(f"{guinfo} | Author: {inter.author.global_name or inter.author.name} | {inter.author.id} | Cus-id: {inter.data['custom_id']}", type="execute component")

    if inter.type == cog.InteractionType.modal_submit:
      arguments = {}
      num=0
      for value in inter.data["components"]:
        arguments[value["components"][0]["custom_id"]] = value["components"][0]["value"]
        num += 1
      if inter.guild:
        guinfo = f"Guild name: '{inter.guild.name}' | Guild id: {inter.guild.id}"
      else:
      ...
      self.bot.logging.info(f"{guinfo} | Author: {inter.author.global_name or inter.author.name} | {inter.author.id} | Cus-id: {inter.data['custom_id']} | Inserts: {arguments}", type="complete modal")
    ...
    if inter.type == cog.InteractionType.application_command_autocomplete:
      ...
      self.bot.logging.info(f"{guinfo} | Author: {inter.author.global_name or inter.author.name} | {inter.author.id} | Name: {inter.data['name']}", type="autocomplete")
