...
class Rating(cog.Cog):
    ...
    @rating.sub_command(
        ...
        description=cog.Localised(
            "Look at someone else's or your rating", key="rating_view_desc"
        ),
    )
    async def view(
        self,
    ...
    async def add(
        ...
        await self.bot.database.updateSocialRating(user_id=view.id, to_rating=newRating)
