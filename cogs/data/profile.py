...
class SettingModal(cog.ui.Modal):
  def __init__(self, 
    ...
    comp = [
      cog.ui.TextInput(
            ...
            placeholder=self.bot.translate(inter, 'profile.edit_modal.color.placeholder'),
            required=False,
      ...
      cog.ui.TextInput(
            ...
            placeholder=self.bot.translate(inter, 'profile.edit_modal.about_me.placeholder'),
            required=False,
      ...
      cog.ui.TextInput(
            ...
            placeholder=self.bot.translate(inter, 'profile.edit_modal.image.placeholder'),
            required=False,
    ...
    super().__init__(title=self.bot.translate(inter, "profile.edit_modal.title"), components=comp)
  ...
  async def callback(self, inter: cog.ModalInteraction):
    ...
    if call['hexcolor'] == '':
      updates["color"] = None
      done.append(self.bot.translate(inter, "profile.edit_modal.res.color_removed"))
    elif call['hexcolor'] != self._profile_color:
      updates["color"] = call["hexcolor"]
      done.append(self.bot.translate(inter, "profile.edit_modal.res.color_changed", color=call['hexcolor']))
    
    if call["about"] == "":
      updates["about_me"] = None
      done.append(self.bot.translate(inter, "profile.edit_modal.res.about_removed"))
    elif call['about'] != self._profile_about:
      updates["about_me"] = call['about']
      done.append(self.bot.translate(inter, "profile.edit_modal.res.about_updated"))
    
    if call["custom_image"] == "":
      updates["custom_image"] = None
      done.append(self.bot.translate(inter, "profile.edit_modal.res.image_removed"))
    elif call['custom_image'] != self._profile_image:
      updates["custom_image"] = call["custom_image"]
      done.append(self.bot.translate(inter, "profile.edit_modal.res.image_updated"))

    try:
      display_color = int(updates["color"], 16)
    except:
      display_color = self.bot.config.colors.blurple
      
      updates["color"] = self.bot.config.colors.blurple

    profile = await self.bot.database.readUserProfiles(user_id=inter.author.id, cacheOverwrite=True)
    ...
    profile = await self.bot.database.readUserProfiles(user_id=inter.author.id, cacheOverwrite=True)
    if not profile:
      return await inter.response.send_message(self.bot.translate(inter, "profile.edit_modal.res.unknown_error"))
    ...
    await channel.send(
      content=self.bot.translate(
        inter, 
        "profile.edit_modal.res.log_message",
        user_id=inter.author.id
      ),
      embed=membership
    )
    await inter.send(
      self.bot.translate(inter, "profile.edti_modal.res.final_updates", updates="\n".join(done)),
      ephemeral=True
    )
...
class Main(cog.Cog):
  ...
  @profile.sub_command(name="timezone",
    description=cog.Localized(key="user_profile_timezone")
  )
  async def timezone(self, inter: cog.ApplicationCommandInteraction, zone: str = cog.Param(
      description=cog.Localized(key="timezone_param_desc"),
      ...
      # Сохраняем в базу
      usersettings = await self.bot.database.readUserSettings(inter.author.id, cacheOverwrite=True)
      if usersettings:
          await self.bot.database.updateUserSettings(user_id=inter.author.id, timezone=zone)
      else:
          await self.bot.database.createUserSettings(user_id=inter.author.id, timezone=zone)

      await inter.response.send_message(
          self.bot.translate(inter, "timezone_set", zone=zone, time=timezone_obj.strftime('%d/%m/%Y, %H:%M:%S'))
      )

  @profile.sub_command(name='preview',description=cog.Localized(key="profile.preview.meta.description"))   
  async def control(self, inter: cog.ApplicationCommandInteraction):
    ...
    edit_buttons = [
      cog.ui.Button(
        label=self.bot.translate(inter, "profile.preview.resp.edit_profile_button"), 
    ...
    if not profile:
      return await inter.edit_original_response(
        content=self.bot.translate(inter, "profile.preview.resp.content.error"),
