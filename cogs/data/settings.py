...
_SS_ROW = """
SELECT
	guild_id, mute_role, join_channel, leave_channel, post_channel, auto_role,
	private_cr_channel, private_category, log_channel, average_language,
	custom_greet, custom_fare, greet_color, greet_image, fare_color, fare_image,
	ticket_category, verified_role, bad_words, bad_words_action,
	notified_moderators, notify_channel, notify_text, fare_text
...
class Settings(cog.Cog):
	...
	async def on_member_join(self, member: cog.Member):
		...
		row = await self.bot.database.sql_fetchone(
			"""
			SELECT join_channel, custom_greet, greet_color, greet_image, auto_role
		...
		join_channel_id, custom_greet, greet_color, greet_image, auto_role = row
		if auto_role:
			try:
				r = member.guild.get_role(int(auto_role))
				if r:
		...
		try:
			channel = await member.guild.fetch_channel(int(join_channel_id))
		except Exception:
			try:
		...
		try:
			done = await self._render_template(custom_greet, member)
			color = _parse_color(greet_color, 0x82E868)
			embed = cog.Embed(
				title=f"[ + ] {cog.utils.escape_markdown(member.display_name)}",
				description=done,
				color=color,
