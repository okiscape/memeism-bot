...
{{server.owner}}""",
    ...
    async def callback(self, inter: cog.ModalInteraction):
        ...
        await self.bot.database.updateServerSettings(
            guild_id, **{self.column: inter.text_values["inputted"]}
        )
...
class EmbedGen(cog.ui.Modal):
    def __init__(self, inter: cog.ApplicationCommandInteraction, bot: cog.KisaBot):
        self.bot = bot
        super().__init__(
            title="Пост",
            components=[
                cog.ui.TextInput(
                    label="Текст поста", custom_id="message_content", required=False
                ),
                cog.ui.TextInput(
                    label="Заголовок эмбэда", custom_id="embed_title", required=False
                ),
                cog.ui.TextInput(
                    label="Текст эмбэда",
                ...
                cog.ui.TextInput(
                    label="Картинка эмбэда", custom_id="embed_image", required=False
                ),
            ],
    ...
    async def callback(self, inter: cog.ModalInteraction):
        ...
        sslist = await self.bot.database.readServerSettings(inter.guild_id)
        if not sslist:
            await self.bot.database.createServerSettings(guild_id=inter.guild_id)
            sslist = await self.bot.database.readServerSettings(
                inter.guild_id, cacheOverwrite=True
            )

        post_channel_id = sslist[0].post_channel if sslist else None
        if not post_channel_id:
            await inter.edit_original_response(
                "Не отправлено, канал для постов не установлен!"
            )
            return
        ...
        if not channel or not isinstance(channel, disnake.TextChannel):
            await inter.edit_original_response(
                "Не отправлено: канал для постов недоступен."
            )
            return
...
class Settings(cog.Cog):
    ...
    async def _render_template(self, template: str, member: cog.Member) -> str:
        mc = getattr(member.guild, "member_count", None) or len(
            getattr(member.guild, "members", []) or []
        )
        owner = member.guild.owner
        owner_mention = owner.mention if owner else ""
        return cog.sh.replace_placeholders(
            template,
            {
                "{{member.mention}}": member.mention,
                "{{member.name}}": await self.bot.shortcuts.rmark(member.display_name),
                "{{server.name}}": await self.bot.shortcuts.rmark(member.guild.name),
                "{{server.members}}": str(mc),
                "{{server.owner}}": await self.bot.shortcuts.rmark(owner_mention),
            },
        )
    ...
    async def setting_cmd(self, inter: cog.ApplicationCommandInteraction):
        ...
        auto_role = s.auto_role
        private_cr_channel = s.private_cr_channel
        private_category = s.private_category
        logs_channel = s.log_channel
        average_language = s.average_language
        custom_greet = s.custom_greet
        custom_fare = s.custom_fare
        greet_color = s.greet_color
        greet_image = s.greet_image
        fare_color = s.fare_color
        fare_image = s.fare_image
        ticket_category = s.ticket_category
        verified_role = s.verified_role
        bad_words = s.bad_words
        bad_action = s.bad_words_action
        _notified_mods = s.notified_moderators
        _notify_ch = s.notify_channel
        _notify_text = s.notify_text
        _fare_text = s.fare_text
        ...
        verce = ""
        verce_list = await self.bot.database.readSerververse(inter.guild.id, limit=1)
        if verce_list:
            v = verce_list[0]
            guild_id1 = v.guild_id1
            guild_id2 = v.guild_id2
            channel1 = v.channel_id1
            channel2 = v.channel_id2
            try:
        ...
        try:
            settings["private_create_channel"] = _ch_mention(
                settings["private_create_channel"]
            )
        except Exception:
        ...
        try:
            settings["language"] = langcodes.get(
        ...
        if settings["custom_greet_text"]:
            ...
            done_greet = cog.sh.replace_placeholders(
                greet,
                {
                    "{{member.mention}}": inter.author.mention,
                    "{{member.name}}": await self.bot.shortcuts.rmark(
                        inter.author.display_name
                    ),
                    "{{server.name}}": await self.bot.shortcuts.rmark(inter.guild.name),
                    "{{server.members}}": str(len(inter.guild.members)),
                    "{{server.owner}}": await self.bot.shortcuts.rmark(
        ...
        if settings["custom_fare_text"]:
            ...
            done_fare = cog.sh.replace_placeholders(
                fare,
                {
                    "{{member.mention}}": inter.author.mention,
                    "{{member.name}}": await self.bot.shortcuts.rmark(
    ...
    @guild.sub_command(
        name="reset", description="Сброс параметра настроек или всего сервера"
    )
    @cog.has_permissions(administrator=True)
    async def reset(
        self,
    ...
    async def on_button_click(self, inter: cog.MessageInteraction):
        if inter.component.custom_id != "server_restart_setting":
            return
        await inter.response.defer(ephemeral=True)
        await self.bot.database.deleteServerSettings(inter.guild_id)
        await inter.edit_original_response(
            "Настройки сервера удалены.", components=None
        )
    ...
    async def guild_mute(
        ...
        sslist = await self.bot.database.readServerSettings(inter.guild_id)
        mute_rid = sslist[0].mute_role if sslist else None
        if not mute_rid:
            await inter.edit_original_response(
    ...
    @s3t.sub_command(
        name="annoucment_channel", description="Канал для постов / оповещений"
    )
    @cog.has_permissions(administrator=True)
    async def set_annoucment(
        ...
        channel: cog.TextChannel = cog.Param(
            description="Канал оповещений будет занесен в базу"
        ),
    ):
        await inter.response.defer(ephemeral=True)
        await self._ensure_row(inter.guild_id)
        await self.bot.database.updateServerSettings(
            inter.guild_id, post_channel=channel.id
        )
        await inter.edit_original_response(
            f"Канал оповещений установлен на <#{channel.id}>"
        )
    ...
    async def set_farewell_image(
        ...
        await inter.response.defer(ephemeral=True)
        await self._ensure_row(inter.guild_id)
        await self.bot.database.updateServerSettings(inter.guild_id, fare_image=image)
        await inter.edit_original_response(
            f"Баннер для прощаний установлен[.]({image})"
        )
    ...
    async def set_greet_image(
        ...
        await inter.response.defer(ephemeral=True)
        await self._ensure_row(inter.guild_id)
        await self.bot.database.updateServerSettings(inter.guild_id, greet_image=image)
        await inter.edit_original_response(
            f"Баннер для приветствий установлен[.]({image})"
        )
    ...
    async def set_greet_color(
        ...
        try:
            ...
            await inter.edit_original_response(
                "Не удалось сохранить цвет! Проверьте формат!"
            )
            return
        await self.bot.database.updateServerSettings(
            inter.guild_id, greet_color=str(done_color)
        )
        await inter.edit_original_response(
            embed=cog.Embed(
                title=f"Цвет приветствий установлен на: #{hex_code}", color=done_color
            )
        )
    ...
    async def set_farewell_color(
        ...
        try:
            ...
            return
        await self.bot.database.updateServerSettings(
            inter.guild_id, fare_color=str(done_color)
        )
        await inter.edit_original_response(
            embed=cog.Embed(
                title=f"Цвет прощаний установлен на: #{hex_code}", color=done_color
            )
        )
    ...
    async def set_language(
        ...
        if not langcodes.tag_is_valid(code):
            await inter.edit_original_response(
                "Пожалуйста введите корректный код языка!"
            )
            return
        await self._ensure_row(inter.guild_id)
        await self.bot.database.updateServerSettings(
            inter.guild_id, average_language=code
        )
        await inter.edit_original_response(
    ...
    async def set_greetings_channel(
        ...
        inter: cog.ApplicationCommandInteraction,
        channel: cog.TextChannel = cog.Param(
            description="Канал приветствий будет занесен в базу"
        ),
    ):
        await inter.response.defer(ephemeral=True)
        await self._ensure_row(inter.guild_id)
        await self.bot.database.updateServerSettings(
            inter.guild_id, join_channel=channel.id
        )
        await inter.edit_original_response(
            f"Канал приветствий установлен на <#{channel.id}>"
        )
    ...
    async def add_bad_word(
        ...
        await self._ensure_row(inter.guild_id)
        sslist = await self.bot.database.readServerSettings(inter.guild_id)
        old = (sslist[0].bad_words if sslist and sslist[0].bad_words else "") or ""
        old_words = (
            (old.strip() + " " + word_s.strip()).strip()
            if old.strip()
            else word_s.strip()
        )
        await self.bot.database.updateServerSettings(
            inter.guild_id, bad_words=old_words
        )
    ...
    @s3t.sub_command(
        name="private_create", description="Голосовой канал для создания приваток"
    )
    @cog.has_permissions(administrator=True)
    async def set_private_create(
        self,
        inter: cog.ApplicationCommandInteraction,
        channel: cog.VoiceChannel = cog.Param(
            description="Канал для создания приватки будет занесен в базу"
        ),
    ):
        await inter.response.defer(ephemeral=True)
        await self._ensure_row(inter.guild_id)
        await self.bot.database.updateServerSettings(
            inter.guild_id, private_cr_channel=channel.id
        )
        sslist = await self.bot.database.readServerSettings(
            inter.guild_id, cacheOverwrite=True
        )
        pcc = sslist[0].private_cr_channel if sslist else channel.id
        await inter.edit_original_response(
            f"Канал для создания приватного голосового канала установлен на <#{pcc}>"
        )

    @s3t.sub_command(
        name="private_category", description="Категория для приватных голосовых"
    )
    @cog.has_permissions(administrator=True)
    async def set_private_category(
        self,
        inter: cog.ApplicationCommandInteraction,
        category: cog.CategoryChannel = cog.Param(
            description="Категория будет занесена в базу"
        ),
    ):
        await inter.response.defer(ephemeral=True)
        await self._ensure_row(inter.guild_id)
        await self.bot.database.updateServerSettings(
            inter.guild_id, private_category=category.id
        )
        await inter.edit_original_response(
    ...
    async def set_notify_channel(
        ...
        inter: cog.ApplicationCommandInteraction,
        channel: cog.TextChannel = cog.Param(
            description="Канал для уведомлений от бота"
        ),
    ):
        await inter.response.defer(ephemeral=True)
        await self._ensure_row(inter.guild_id)
        await self.bot.database.updateServerSettings(
            inter.guild_id, notify_channel=channel.id
        )
        await inter.edit_original_response(
            f"Канал для уведомлений установлен - {channel.mention}"
        )
    ...
    async def set_farewell_channel(
        ...
        inter: cog.ApplicationCommandInteraction,
        channel: cog.TextChannel = cog.Param(
            description="Канал прощаний будет занесен в базу"
        ),
    ):
        await inter.response.defer(ephemeral=True)
        await self._ensure_row(inter.guild_id)
        await self.bot.database.updateServerSettings(
            inter.guild_id, leave_channel=channel.id
        )
        await inter.edit_original_response(
            f"Канал прощаний установлен на {channel.mention}"
        )

    @s3t.sub_command(
        name="set_bad_word_penalty", description="Мера наказания за запрещённые слова"
    )
    @cog.has_permissions(administrator=True)
    async def set_bad_word_penalty(
        ...
        if type != "info":
            await self._ensure_row(inter.guild_id)
            await self.bot.database.updateServerSettings(
                inter.guild_id, bad_words_action=type
            )
            await inter.edit_original_response(
                f"Тип наказаний за запрещенные слова установлен на `{type}`"
            )
        else:
            await inter.edit_original_response(
                embed=cog.Embed(
    ...
    async def set_autorole(
        self,
        inter: cog.ApplicationCommandInteraction,
        role: cog.Role = cog.Param(
            description="Эта роль будет выдаваться каждому новому участнику"
        ),
    ):
        await inter.response.defer(ephemeral=True)
        await self._ensure_row(inter.guild_id)
        await self.bot.database.updateServerSettings(inter.guild_id, auto_role=role.id)
        ...
        if bot_tr and role.position > bot_tr.position:
            ...
            await inter.edit_original_response(
                f"Теперь роль {role.mention} будет иметь каждый новый участник"
            )
    ...
    async def mute_role_cmd(
        ...
        inter: cog.ApplicationCommandInteraction,
        role: cog.Role = cog.Param(
            description="Роль которая будет даваться на время по команде /guild mute"
        ),
    ):
        await inter.response.defer(ephemeral=True)
        try:
            role = inter.guild.get_role(
                cog.utils.get(inter.guild.roles, name=role.name).id
            )
        except Exception:
            pass
        await self._ensure_row(inter.guild_id)
        await self.bot.database.updateServerSettings(inter.guild_id, mute_role=role.id)
        bot_tr = self._bot_top_role(inter.guild)
        if bot_tr and role.position > bot_tr.position:
            err = cog.Embed(
                color=0xDB553F,
            ...
            await inter.edit_original_response(
                f"Ролью мута теперь является <@&{role.id}>"
            )
    ...
    async def logs_channel_cmd(
        ...
        await self._ensure_row(inter.guild_id)
        await self.bot.database.updateServerSettings(
            inter.guild_id, log_channel=channel.id
        )
        await inter.edit_original_response(
            f"Каналом логов теперь является -> <#{channel.id}>"
        )
    ...
    async def set_verified_role(
        ...
        inter: cog.ApplicationCommandInteraction,
        role: cog.Role = cog.Param(
            description="Роль, которая будет выдаваться после верификации"
        ),
    ):
        await inter.response.defer(ephemeral=True)
        try:
        ...
        await self._ensure_row(inter.guild_id)
        await self.bot.database.updateServerSettings(
            inter.guild_id, verified_role=role.id
        )
        sslist = await self.bot.database.readServerSettings(
            inter.guild_id, cacheOverwrite=True
        )
        vr = sslist[0].verified_role if sslist else role.id
        ar = sslist[0].auto_role if sslist else None
        bot_tr = self._bot_top_role(inter.guild)
        if bot_tr and role.position > bot_tr.position:
            err = cog.Embed(
                color=0xDB553F,
            ...
            await inter.edit_original_response(
                f"Ролью верификации теперь является <@&{vr}>"
            )
    ...
    async def set_ticket_category(
        ...
        await self._ensure_row(inter.guild_id)
        await self.bot.database.updateServerSettings(
            inter.guild_id, ticket_category=category.id
        )
        await inter.edit_original_response(
            f"Категория тикетов установлена: {category.mention}"
        )

    @s3t.sub_command(
        name="notified_moderators",
        description="ID модераторов для уведомлений (через пробел)",
    )
    @cog.has_permissions(administrator=True)
    async def set_notified_moderators(
        ...
        await self._ensure_row(inter.guild_id)
        await self.bot.database.updateServerSettings(
            inter.guild_id, notified_moderators=moderator_ids.strip()
        )
    ...
    async def on_member_remove(self, member: cog.Member):
        ...
        sslist = await self.bot.database.readServerSettings(member.guild.id)
        if not sslist:
            return

        s = sslist[0]
        leave_channel_id = s.leave_channel
        custom_fare = s.custom_fare
        fare_color = s.fare_color
        fare_image = s.fare_image
